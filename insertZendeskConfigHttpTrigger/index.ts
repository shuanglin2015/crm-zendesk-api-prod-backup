...
import searchService from '../shared/services/searchForConfigTimerTriggerService';
import util from '../shared/utils/util';
import crmUtil from '../shared/utils/crmUtil';

const httpTrigger: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    const log: Logger = context.log;
    log('HTTP trigger insertZendeskConfigsHttpTrigger function processed a request.');
    ...
    try {
        ...
        const items = await searchService.retrieveData(log, updatedDateStart, updatedDateEnd);
        ...
        context.res = {
            status: 500,
            body: "Error occurred in insertZendeskConfigHttpTrigger API - Please check logs to find the details"
        };
    }
